...
class VideoSavor:
    ...
    def __init__(self, targetFileName, videoCapture):
        ...
        self.outputStream = cv2.VideoWriter(
            targetFileName + ".avi", codec, frameRate, (width, height))
...
def DetectViolence(PATH_FILE_NAME_OF_SOURCE_VIDEO, PATH_FILE_NAME_TO_SAVE_RESULT):
    ...
    startTime = time.time()
    if shouldSaveResult:
        videoSavor = VideoSavor(
            PATH_FILE_NAME_TO_SAVE_RESULT + "_Result", videoReader)
    ...
    while isCurrentFrameValid:
        nowTime = time.time()
        if ((nowTime - startTime)) > num_frames:
...
if __name__ == '__main__':
    if len(sys.argv) >= 2:
        ...
        if os.path.isfile(PATH_FILE_NAME_OF_SOURCE_VIDEO):
            DetectViolence(PATH_FILE_NAME_OF_SOURCE_VIDEO,
        ...
        PATH_FILE_NAME_TO_SAVE_RESULT = None
        PATH_FILE_NAME_OF_SOURCE_VIDEO = 0
        DetectViolence(PATH_FILE_NAME_OF_SOURCE_VIDEO,
                       PATH_FILE_NAME_TO_SAVE_RESULT)
